...
/// Shortcuts-compatible intent — sends a query to PocketMind AI and returns the response.
/// Unlike AskPocketMindIntent (which opens the app), this runs in background and returns text.
/// Note: This uses a simplified LLM interaction - for full context-aware conversations, use the app.
struct AskPocketMindShortcutIntent: AppIntent {

    static let title: LocalizedStringResource = "Ask PocketMind (Background)"
    ...
    func perform() async throws -> some IntentResult & ReturnsValue<String> & ProvidesDialog {
        ...
        let response = "Question saved: \"\(trimmed)\". Opening PocketMind for AI response."
