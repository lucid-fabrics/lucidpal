...
final class ChatViewModelTests: XCTestCase {
    ...
    func testConfirmDeletionSetsStateToDeleted() async throws {
        ...
        XCTAssertEqual(viewModel.messages.first?.calendarEventPreviews.first?.state, .deleted)
    ...
    func testConfirmDeletionWithNilIdentifierIsNoOp() async throws {
        ...
        await viewModel.confirmDeletion(messageID: msgID, previewID: previewID)
        XCTAssertEqual(viewModel.messages.first?.calendarEventPreviews.first?.state, .pendingDeletion)
    ...
    func testCancelDeletionSetsStateToCancelled() {
        ...
        XCTAssertEqual(viewModel.messages.first?.calendarEventPreviews.first?.state, .deletionCancelled)
        XCTAssertTrue(mock.deletedIdentifiers.isEmpty)
    }
    ...
    func testUndoDeletionSetsStateToRestored() async throws {
        ...
        XCTAssertEqual(viewModel.messages.first?.calendarEventPreviews.first?.state, .restored)
    ...
    func testConfirmAllDeletionsSetsAllToDeleted() async throws {
        var msg = ChatMessage(role: .assistant, content: "")
        msg.calendarEventPreviews = [
            CalendarEventPreview(title: "A", start: .now, end: .now, calendarName: nil, state: .pendingDeletion, eventIdentifier: "id-1"),
            CalendarEventPreview(title: "B", start: .now, end: .now, calendarName: nil, state: .pendingDeletion, eventIdentifier: "id-2"),
        ]
        viewModel.messages.append(msg)
        await viewModel.confirmAllDeletions(messageID: msg.id)
        XCTAssertEqual(viewModel.messages.first?.calendarEventPreviews.map(\.state), [.deleted, .deleted])
    ...
    func testCancelAllDeletionsSetsAllToCancelled() {
        ...
        XCTAssertEqual(viewModel.messages.first?.calendarEventPreviews.map(\.state), [.deletionCancelled, .deletionCancelled])
    }
    ...
    func testConfirmUpdateEventNotFoundSetsErrorMessage() async throws {
        ...
        let (msgID, previewID) = insertMessage(state: .pendingUpdate, pendingUpdate: pending)
        await viewModel.confirmUpdate(messageID: msgID, previewID: previewID)
        XCTAssertEqual(viewModel.messages.first?.calendarEventPreviews.first?.state, .updateCancelled)
        let errorMsg = try XCTUnwrap(viewModel.errorMessage)
        XCTAssertFalse(errorMsg.isEmpty)
    }
    ...
    func testCancelUpdateWithWrongIDsIsNoOp() {
        ...
        XCTAssertEqual(viewModel.messages.first?.calendarEventPreviews.first?.state, before)
    }
    ...
    func testCancelDeletionDoesNotAffectAlreadyDeletedPreviews() {
        var msg = ChatMessage(role: .assistant, content: "")
        msg.calendarEventPreviews = [
            CalendarEventPreview(title: "A", start: .now, end: .now, calendarName: nil, state: .deleted, eventIdentifier: "id-1"),
            CalendarEventPreview(title: "B", start: .now, end: .now, calendarName: nil, state: .pendingDeletion, eventIdentifier: "id-2"),
        ]
        viewModel.messages.append(msg)
        viewModel.cancelAllDeletions(messageID: msg.id)
