...
All notable changes to LucidPal iOS will be documented in this file.
...
- **Shortcuts App Integration**: Expose LucidPal actions to iOS Shortcuts for automation
  ...
  - `Ask LucidPal (Background)` - Quick query that opens app with pre-filled question
  ...
  - 4 new `AppIntent` types: `CreateEventShortcutIntent`, `CheckNextMeetingIntent`, `FindFreeTimeShortcutIntent`, `AskLucidPalShortcutIntent`
...
- `LucidPalApp`: Initialized `AudioRouteMonitor` and `AirPodsVoiceCoordinator` services
- `SessionListViewModel`: Added `AirPodsVoiceCoordinator` dependency for ChatViewModel factory
- `LucidPalShortcuts`: Added 4 new shortcut entries with Siri phrases
