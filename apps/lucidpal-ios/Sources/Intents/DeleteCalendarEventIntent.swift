...
private let intentLogger = Logger(subsystem: "app.lucidpal", category: "DeleteCalendarEventIntent")

// MARK: - DeleteCalendarEventIntent

/// Siri intent: "Delete [event] in LucidPal"
...
struct DeleteCalendarEventIntent: AppIntent {
    ...
    static let description = IntentDescription("Delete an event from your calendar via LucidPal")
...
/// Siri intent: "Undo my last LucidPal action"
...
struct UndoLastDeletionIntent: AppIntent {
    ...
    static let description = IntentDescription("Undo the last calendar action taken in LucidPal")
    static let openAppWhenRun: Bool = false
    ...
    func perform() async throws -> some IntentResult & ProvidesDialog & ShowsSnippetView {
        ...
        case .updated, .rescheduled:
            return .result(
                dialog: "I can't undo event edits yet — open LucidPal to make changes manually.",
                view: SiriEventCard(
                    title: last.eventTitle, start: last.eventStart, end: last.eventEnd,
                    calendarName: last.calendarName, isAllDay: last.isAllDay, deleted: false
                )
