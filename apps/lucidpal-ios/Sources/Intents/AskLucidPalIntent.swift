...
private let intentLogger = Logger(subsystem: "app.lucidpal", category: "AskLucidPalIntent")
...
enum SiriQueryError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .emptyQuery: return "Please provide a question for LucidPal."
        }
    }
}

// MARK: - AskLucidPalIntent

/// Siri intent — user says "Ask LucidPal [query]".
/// The query is stored in UserDefaults; LucidPalApp picks it up
/// when the scene becomes active and forwards it to ChatViewModel.
struct AskLucidPalIntent: AppIntent {

    static let title: LocalizedStringResource = "Ask LucidPal"
    static let description = IntentDescription("Ask your on-device AI assistant a question")
    static let openAppWhenRun: Bool = true

    @Parameter(title: "Question",
               description: "What would you like to ask LucidPal?",
...
/// Siri intent — user says "Check my LucidPal calendar".
...
struct CheckCalendarIntent: AppIntent {
    ...
    static let description = IntentDescription("See what's on your calendar via LucidPal")
    static let openAppWhenRun: Bool = true
...
/// Siri intent — user says "Add [event] to LucidPal".
...
struct AddCalendarEventIntent: AppIntent {
    ...
    static let description = IntentDescription("Add a new event to your calendar via LucidPal")
    static let openAppWhenRun: Bool = true
    ...
    func perform() async throws -> some IntentResult & ProvidesDialog {
        ...
        return .result(dialog: "Opening LucidPal to schedule \(trimmed).")
    }
}

// MARK: - FindFreeTimeIntent

/// Siri intent — user says "Find free time in LucidPal".
...
struct FindFreeTimeIntent: AppIntent {
    ...
    static let description = IntentDescription("Find a free slot in your schedule via LucidPal")
    static let openAppWhenRun: Bool = true
