...
private let searchLogger = Logger(subsystem: "app.lucidpal", category: "WebSearch")
...
final class WebSearchService: WebSearchServiceProtocol {
    ...
    func search(query: String, maxResults: Int = 5) async throws -> [WebSearchResult] {
        ...
        searchLogger.info("🔍 SEARCH [\(provider, privacy: .public)]: \"\(query, privacy: .private)\" → \(results.count) results")
        return results
    }
