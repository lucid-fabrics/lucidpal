...
private let systemPromptLogger = Logger(subsystem: "app.lucidpal", category: "Chat")
...
final class SystemPromptBuilder: SystemPromptBuilderProtocol {
    ...
    // Matches [CALENDAR_ACTION:{...}] — supports one level of nested objects (e.g. recurrence fields).
    // Pattern: outer { ... } where content is any non-brace char or a nested { non-brace chars }.
    ...
    // Matches [WEB_SEARCH:{...}] — supports one level of nested objects (same as actionPattern).
